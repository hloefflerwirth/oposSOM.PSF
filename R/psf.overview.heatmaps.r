...
psf.overview.heatmaps <- function(psf.results)
{
  
  filename <- file.path(output.paths["PSF"], "0verview Heatmaps.pdf")
  ...
  my.heatmap(x=log1p(log1p(mean.psf.matrix)), cex.main=2,
  ...
  mtext("mean signal", cex=1.4, line=33)
  ...
  my.heatmap(x=log1p(log1p(max.psf.matrix)), cex.main=2,
             col=colorRampPalette(c("blue4","blue","gray90","orange","red4"))(1000),scale="r",
             mar=c(10,20), ColSideColors=group.colors, cexDend=0.6 )
